...
// 培养计划：玩家在状态栏家族页设置（方向为空=未培养，AI 勿改）；「产业结算脚本」每周期按此扣款并给属性成长。
const 家族成员条 = z.object({
  ...
  基础属性: 基础属性块,
  培养计划: z.object({
    方向: str('').transform(v => ['', '武技', '魔法', '学识', '商业', '骑术', '信仰', '综合'].includes(v) ? v : ''),
    每期投入: safeNum(0),   // 铜盾/月；0=仅挂名不投入（无成长）
    成长进度: safeNum(0),   // 脚本托管：本月累计成长点（满12点=对应属性+1），AI 勿改
