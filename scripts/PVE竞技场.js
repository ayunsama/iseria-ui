...
@media (max-width:786px){
  ...
  #arx-meCard{ max-height:200px; padding:10px 12px; }
...
8. 完整推演纪律（最高优先级）：从【第1轮】到最后战斗结束，**每一轮都必须输出独立的【第N轮】小节**，每个小节包含每个行动者 1~3 句行动叙事 + 该轮全部关键结算行。严禁"省略/略过/第X~Y轮合并/直接跳到结局/只写关键轮次"等任何形式的跳过；叙事保持紧凑（每轮合计不超过 120 字）以保障完整度。
9. 必须分出胜负：随轮次推进合理提升命中与伤害（破防/机制触发/协力集火），在 12 轮内结束战斗；仅当双方确实完全无法互伤时才允许平局。

═══ 输出协议（严格三段、顺序固定、缺一不可）═══
<构筑推演>
（内嵌思维链一：数据卡整理与战力对比。若选手没有完整数据卡，按其描述与等阶，参照位阶表与同类单位数值现场生成完整数据卡：[名称][等阶][等级][HP|当前|最大][防御值][属性六维][攻击|名|类型|命中修正|伤害骰|特效][能力][机制?][弱点/抗性/免疫?][奥义（史诗级以上必备）][权能（传说级以上必备）]，逐项写明推导理由。随后给出双方战力对比与胜负手分析，以及先攻预估。标签后第一行就是推演正文；本段必须至少 200 字实质内容，禁止输出空标签壳。）
</构筑推演>
<战斗推演>
（内嵌思维链二：逐回合推演。先攻掷骰算式→按行动序逐人推演：行动选择逻辑（基于数据卡能力与当前态势）→每次攻击/施法的掷骰算式与命中判定→伤害计算全过程→HP扣减台账（名：旧值→新值）→机制状态核对。战斗推演的轮次数必须与战报完全一致，一轮都不许省。标签后第一行就是推演正文；本段必须至少 200 字实质内容，禁止输出空标签壳。）
</战斗推演>
<战报>
...
现在开始推演，严格按输出协议输出三段。`;
    ...
    /* ================== 战报渲染 ================== */
    /** 案例：三段协议的宽松提取——真实模型常不守标签协议（漏闭合/用【】/全混正文/输出空标签壳），逐级降级兜底 */
    const ARENA_MIN_SEG = 30; // 段有效最短字数：低于此视为空壳（模型输出了标签但没写内容）
    function cleanArenaSeg(s) {
        return String(s ?? '')
            .replace(/<\/?(?:构筑推演|战斗推演|战报)>/gi, '')
            .replace(/\[(?:构筑推演|战斗推演|战报)\]/g, '')
            .trim();
    }
    function extractArenaSections(out) {
        const seg = (tag) => {
            // 1) 标准闭合标签（非贪婪取第一对；若第一对是空壳继续向后找）
            const re1 = new RegExp('<' + tag + '>([\\s\\S]*?)</' + tag + '>', 'gi');
            let m;
            while ((m = re1.exec(out))) {
                const c = cleanArenaSeg(m[1]);
                if (c.length >= ARENA_MIN_SEG) return c;
            }
            // 2) 有开无闭（吃到下一个输出标签或文末）
            const re2 = new RegExp('<' + tag + '>([\\s\\S]*?)(?=<(?:构筑推演|战斗推演|战报)>|【(?:构筑推演|战斗推演|战报)】|$)', 'i');
            if ((m = re2.exec(out))) {
                const c = cleanArenaSeg(m[1]);
                if (c.length >= ARENA_MIN_SEG) return c;
            }
            // 3) 【tag】变体（吃到下一个变体/标签/文末）
            const re3 = new RegExp('【' + tag + '】([\\s\\S]*?)(?=<(?:构筑推演|战斗推演|战报)>|【(?:构筑推演|战斗推演|战报)】|$)', 'i');
            if ((m = re3.exec(out))) {
                const c = cleanArenaSeg(m[1]);
                if (c.length >= ARENA_MIN_SEG) return c;
            }
        ...
        const reportStart = () => {
            let i = out.search(/<战报>|【战报】|\[战报\]/i);
            if (i < 0) i = out.search(/⚔/);
            return i;
        };
        // 兜底 A：战报段无效 → 锚点之后的文本即战报；连锚点都没有 → 全文即战报
        if (!reportTxt) {
            const idx = reportStart();
            reportTxt = cleanArenaSeg(idx >= 0 ? out.slice(idx) : out) || out.trim();
            degraded = true;
        }
        // 兜底 B：推演段无效 → 战报锚点之前的全部正文（剥标签）充作推演块
        if (!buildTxt || !simTxt) {
            const idx = reportStart();
            const pre = idx > 0 ? cleanArenaSeg(out.slice(0, idx)) : '';
            if (pre) {
                const note = '\n（注：模型未严格按协议分段，以上为战报前的全部推演内容）';
                if (!buildTxt) buildTxt = pre + note;
                if (!simTxt) simTxt = pre + note;
                degraded = true;
            }
        }
